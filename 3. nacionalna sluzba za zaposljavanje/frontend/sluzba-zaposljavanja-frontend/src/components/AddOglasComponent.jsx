import {addAdvertisement, getAdvertisementById, updateAdvertisement} from "../services/OglasService";
import React, { useState, useEffect } from "react";
import { getAllFirms } from "../services/FirmaService";
import { getAllJobs } from "../services/VrstaPoslaService";


const AddOglasComponent = () => {
                    ...
                    <div className="card col-md-6 offset-md-3" >
                        ...
                        <form>
                          ...
                          <select className="form-select" name="firma" onChange={handleFirms} style={{width:"300px", margin:"0 auto"}}>
                            {firme.map(firma => <option value={firma.id}>{firma.imeFirme}</option>)}
                          </select>

                          Vrsta posla:
                          <select className="form-select" name="vrstaPosla" onChange={handleJobs} style={{width:"300px", margin:"0 auto"}}>
                            {poslovi.map(posao => <option value={posao.id}>{posao.ime}</option>)}
                          </select>
                        </form>
...
export default AddOglasComponent;